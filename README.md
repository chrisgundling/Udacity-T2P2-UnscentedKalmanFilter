...
One more note about the `ukf.cpp` script is that for dataset2 I needed to reduce the maximum timestep (dt) seen in the measurements. I was getting endlessly stuck within one of the loops in the prediction function due to the large change in timestep between measurements. Sub-dividing the largest timesteps solved this issue. 
